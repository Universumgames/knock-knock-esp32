...
void initPatternManagement() {
    queue = xCreateQueue(8, sizeof(gpio_num_t));
    xTaskCreate(patternManagerThread, "pattern_management_thread", THREAD_STACK_MANAGEMENT,
                NULL, THREAD_PRIO_MANAGEMENT, NULL);
...
static void patternManagerThread(void* arg){
    gpio_num_t pin = 0;
    while(true){
        // check if queue contains item (stored automatically in pin), if not wait
        ...
        if(pin == PIN_BUTTON_PLAY){
            ...
            PatternData* toPlay = list_get(patterns, selectedPatternIndex);
            playPattern(toPlay);
        }else if (pin == PIN_BUTTON_UP){
            // up button was pressed
            selectedPatternIndex = (selectedPatternIndex + patternLength - 1) % patternLength;
           ...
           // down button was pressed
            ...
            PatternData* toDelete = list_get(patterns, selectedPatternIndex);
